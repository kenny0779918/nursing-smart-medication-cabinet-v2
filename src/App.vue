...
import ControlledDispense from './components/ControlledDispense.vue'
...
function logout(){ if(hardwareBusy.value){ window.alert('藥盒作業進行中，請先完成並關閉藥盒。'); return } user.value=null; window.clearTimeout(inactivityTimer); returnToLogin() }
...
function openFunction(item){ resetInactivityTimer(); if(!isAllowed(item.key))return; if(item.key==='DISPENSE'){ page.value='dispense'; return } window.alert(`${item.label}功能將於後續模組開放。`) }
function goHome(){ if(hardwareBusy.value){ window.alert('藥盒作業進行中，請先完成並關閉藥盒。'); return } page.value='home' }
...
<template>
  <div class="app-shell">
    ...
    <ControlledDispense v-else-if="page==='dispense'" :operator="user" @back="goHome" @busy="hardwareBusy=$event" />
